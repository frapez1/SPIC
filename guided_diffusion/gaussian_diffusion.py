...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if 'y' in model_kwargs:
            model_output = model(x, self._scale_timesteps(t), cond=model_kwargs['y'], low_res=model_kwargs['compressed'] )
            # print(x.norm()/model_output.norm())
        else:
            model_output = model(x, self._scale_timesteps(t), **model_kwargs)

        if 's' in model_kwargs and model_kwargs['s'] > 1.0:
            model_output_zero = model(x, self._scale_timesteps(t), cond=th.zeros_like(model_kwargs['y']), low_res=model_kwargs['compressed'])
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), cond=model_kwargs['y'], low_res=model_kwargs['compressed'])
